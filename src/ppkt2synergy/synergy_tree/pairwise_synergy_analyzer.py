...
class PairwiseSynergyAnalyzer:
    ...
    >>> hpo_matrix, disease_status = HPOMatrixProcessor.prepare_hpo_data(
    ...
    def __init__(
        ...
        Parameters:
        hpo_matrix : array-like, pandas.DataFrame
        ...
        n_permutations : int, optional
            Number of permutations for calculating p-values (default: 100).
        min_individuals_for_synergy_caculation : int, optional
            Minimum number of samples required to calculate synergy (default: 40).
        ...
        if not np.all(np.isin(hpo_matrix[~np.isnan(hpo_matrix)], [0, 1])):
            raise ValueError("Non-NaN values in X must be either 0 or 1")
    ...
    def evaluate_pair_synergy(
        ...
        Returns:
        - (i, j, corrected_synergy, p_value): Tuple with synergy score and its significance.
            - corrected_synergy: Observed synergy minus mean permutation synergy.
            - p_value: Empirical p-value (two-sided) based on permutation test.
        """
        mask = (
            ~np.isnan(self.X[:, i]) 
            & ~np.isnan(self.X[:, j]) 
    ...
    def compute_synergy_matrix(
        ...
        """
        Compute the synergy and permutation p-value matrices for all feature pairs.

        Parameters:
        - n_jobs (int): Number of parallel jobs to run. Set to -1 to use all available CPUs.

        Returns:
        - synergy_matrix (ndarray): A symmetric matrix (n_features x n_features) where 
        synergy_matrix[i, j] contains the synergy score between feature i and j.
        - pvalue_matrix (ndarray): A symmetric matrix of empirical p-values corresponding to each synergy value.
        """
        combinations_list = list(combinations(range(self.n_features), 2))
        ...
        self.synergy_matrix = pd.DataFrame(
            self.synergy_matrix[valid_mask][:, valid_mask],
            index=valid_hpo_terms,
            columns=valid_hpo_terms
        )

        self.pvalue_matrix = pd.DataFrame(
            self.pvalue_matrix[valid_mask][:, valid_mask],
            index=valid_hpo_terms,
            columns=valid_hpo_terms
        )
    ...
    def filter_weak_synergy(
        ...
        Args:
            lower_bound (float): The minimum synergy value to keep (default: 0.1).
    ...
    def plot_synergy_heatmap(
        ...
        if synergy_matrix.empty or np.isnan(synergy_matrix.values).all():
            raise ValueError("Synergy matrix is empty or all values are NaN. Nothing to plot.")
        ...
        ax = sns.heatmap(
            ...
            linewidths=0.5,
            linecolor="gray", 
            cbar_kws={"label": "Corrected Synergy"},
            mask=nan_mask,
